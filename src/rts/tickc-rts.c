...
static unsigned int _tc_ninsn;	/* Number of insns generated by last compile */
...
void _tc_jump (void * c) {	/* Icode version */
     ...
     _tc_cspec_t dst = *(_tc_cspec_t *)(_c + 1);
     if (!dst)
...
static struct v_aggregate _tc_direct_result;
static int _tc_has_direct_result;
...
_tc_vspec_t _tc_param (unsigned int type, unsigned int argno,
     ...
     if (type == I_B) return i_paramb(aggregate, argno);
     return i_paramn(type, argno);
}

_tc_vspec_t _tc_paramb(const struct v_aggregate *aggregate,
                       unsigned int argno) {
     if (!_tc_init) {
	  i_init(TC_CODESIZE); ++_tc_init;
     }
     return i_paramb(aggregate, argno);
}
				/* Vcode version */
_tc_vspec_t _tc_paramf (unsigned int type, unsigned int argno,
                        const struct v_aggregate *aggregate) {
     _tc_vspec_t id;
     if (type == I_B) return _tc_parambf(aggregate, argno);
     if (argno >= I_MAXPARAM)
...
_tc_vspec_t _tc_parambf(const struct v_aggregate *aggregate,
                        unsigned int argno) {
     _tc_vspec_t id;
     if (!aggregate || argno >= I_MAXPARAM)
	  v_fatal("param: invalid aggregate parameter.\n");
     id = _tc_localbf(aggregate->size);
     _tc_llocal[id].reg = L_PARM;
     _tc_lparam[argno].local = id;
     _tc_lparam[argno].allocp = 1;
     _tc_lparam[argno].aggregate = *aggregate;
     if (argno >= _tc_curparam) _tc_curparam = argno + 1;
     return id;
}

const struct v_aggregate *_tc_aggregate(unsigned int size,
                                        unsigned int alignment,
                                        unsigned int field_count, ...) {
     struct v_aggregate *aggregate;
     va_list ap;
     unsigned int i;
     if (field_count > V_MAX_AGGREGATE_FIELDS)
	  v_fatal("aggregate: too many fields.\n");
     NEW0(aggregate, 1);
     aggregate->size = size;
     ...
     for (i = 0; i < field_count; i++) {
	  aggregate->fields[i].offset = va_arg(ap, unsigned int);
	  aggregate->fields[i].type = va_arg(ap, unsigned int);
     }
     va_end(ap);
     return aggregate;
}
...
_tc_ip_t _tc_compileb(_tc_cspec_t cs,
                      const struct v_aggregate *aggregate) {
     return _tc_compile(cs, TC_B, aggregate);
}
				/* Vcode version */
_tc_ip_t _tc_compilef (_tc_cspec_t cs, unsigned int type,
                       const struct v_aggregate *aggregate) {
     v_iptr ip;			/* Should be the same as _tc_ip_t */
     unsigned int i;
     int sz;
     ...
     _tc_loc_t *lp;
     unsigned int roff = 0;
     int aggregate_signature = _tc_has_direct_result;
     struct v_parameter parameters[TC_MAXPARAM];
     v_reg_t parameter_values[TC_MAXPARAM];
     if (type == TC_B) {
	  if (!aggregate) v_fatal("compile: missing aggregate result.\n");
	  _tc_direct_result = *aggregate;
	  _tc_has_direct_result = 1;
     ...
     if (_tc_phase == endClosures) {
	  ...
	  _tc_has_direct_result = 0;
	  return (_tc_ip_t)0;	/* For benchmarking closures */
     }
				/* Allocate code buffer, not from the arena */
     _tc_cp = (_tc_code_t)v_code_alloc(TC_CODESIZE+i_cachesize);
     if (_tc_cp == NULL)
     ...
     for (pp=&_tc_lparam[0]; pp < &_tc_lparam[_tc_curparam]; pp++)
	  if (!pp->allocp)
	       v_fatal("compile: parameter %d has not been allocated.\n",
		       pp-&_tc_lparam[0]);
	  else if (_tc_llocal[pp->local].type == TC_B)
	       aggregate_signature = 1;
				/* Start vcode code gen */
     if (aggregate_signature) {
	  int parameter_count = (int)_tc_curparam;
	  int parameter_index;
	  for (parameter_index = 0; parameter_index < parameter_count;
	       parameter_index++) {
	       _tc_loc_t *parameter_local =
		    &_tc_llocal[_tc_lparam[parameter_index].local];
	       parameters[parameter_index].type = parameter_local->type;
	       parameters[parameter_index].aggregate =
		    parameter_local->type == TC_B ?
		    &_tc_lparam[parameter_index].aggregate : NULL;
	  }
	  v_lambda_aggregate("",
	       _tc_has_direct_result ? &_tc_direct_result : NULL,
	       parameters, parameter_count, parameter_values,
	       _tc_leafp ? V_LEAF : V_NLEAF, _tc_cp, TC_CODESIZE);
	  /* Save scalar parameter registers before aggregate copies use scratch
	     registers that may still hold later parameters. */
	  for (parameter_index = 0; parameter_index < parameter_count;
	       parameter_index++) {
	       _tc_loc_t *parameter_local =
		    &_tc_llocal[_tc_lparam[parameter_index].local];
	       if (parameter_local->type != TC_B) {
		    parameter_local->addr = v_local(parameter_local->type);
		    v_emit_store(parameter_local->type,
				 parameter_values[parameter_index], v_lp,
				 parameter_local->addr, 0);
		    v_putreg(parameter_values[parameter_index],
			     parameter_local->type);
	       }
	  }
	  for (parameter_index = 0; parameter_index < parameter_count;
	       parameter_index++) {
	       ...
	       if (parameter_local->type == TC_B) {
		    ...
		    parameter_local->addr = v_localb(parameter_local->size);
		    v_addpi(destination, v_lp, parameter_local->addr);
		    v_copyb(destination, parameter_values[parameter_index],
			    parameter_local->size);
		    v_putreg(parameter_values[parameter_index], V_P);
	       }
	  }
     } else {
	  for (pp=&_tc_lparam[0]; pp < &_tc_lparam[_tc_curparam]; pp++) {
	       lp = &_tc_llocal[pp->local];
	       v_param_alloc((int)(pp - &_tc_lparam[0]), lp->type, &lp->addr);
	  }
...
_tc_ip_t _tc_compilebf(_tc_cspec_t cs,
                       const struct v_aggregate *aggregate) {
     if (!aggregate) v_fatal("compile: invalid aggregate result.\n");
     _tc_direct_result = *aggregate;
     _tc_has_direct_result = 1;
     return _tc_compilef(cs, TC_B, aggregate);
}
...
void _tc_decompile (_tc_code_t cp) {
     ...
     if (e) {
	  if (e->cp == cp) {
	       ...
	       v_code_free((void *)(((char *)e->cp)-e->offset));
	       return;
	  }
	  ...
	  for (n = e->link; n; e = e->link, n = n->link)
	       if (n->cp == cp) {
		    e->link = n->link;
		    v_code_free((void *)(((char *)n->cp)-n->offset));
		    return;
	       }
     }
