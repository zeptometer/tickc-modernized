...
/* A vspec carries an icode local identifier, but it participates in closure
   layouts as an indirect handle.  Keep it target-sized so the compiler and
   the host C compiler agree on LP64 field offsets. */
typedef uintptr_t _tc_vspec_t;
...
#define _tc_stackp(loc) (_tc_llocal[(loc)].addr < 0 || ! _tc_llocal[(loc)].reg)
...
extern _tc_vspec_t _tc_paramb (const struct v_aggregate *aggregate,
                               unsigned int argno);
				/* Vcode support */
...
extern _tc_vspec_t _tc_paramf (unsigned int type, unsigned int argno,
                               const struct v_aggregate *aggregate);
extern _tc_vspec_t _tc_parambf (const struct v_aggregate *aggregate,
                                unsigned int argno);
extern const struct v_aggregate *_tc_aggregate(unsigned int size,
                                                unsigned int alignment,
                                                unsigned int field_count, ...);

/* Dynamic compilation */
				/* Icode */
extern _tc_ip_t _tc_compile (_tc_cspec_t cs, unsigned int type,
                             const struct v_aggregate *aggregate);
extern _tc_ip_t _tc_compileb (_tc_cspec_t cs,
                              const struct v_aggregate *aggregate);
				/* Vcode */
extern _tc_ip_t _tc_compilef (_tc_cspec_t cs, unsigned int type,
                              const struct v_aggregate *aggregate);
extern _tc_ip_t _tc_compilebf (_tc_cspec_t cs,
                               const struct v_aggregate *aggregate);
