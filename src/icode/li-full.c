...
static inline void lriuseiter (i_local_t l, bvt vec, void *v) {
     i_cnt_t pos = (i_cnt_t)(uintptr_t)v;
...
void i_li_full (void) {
     ...
     for (b = i_fg_tail; b; pos--, b = b->lprev) {
	  ...
	  assert(pos+1 == hpos);
	  if (num_i) bv_eachbit(ilv, lriuseiter, (void *)(uintptr_t)hpos);
	  if (num_f) bv_eachbit(flv, lrfuseiter, (void *)(uintptr_t)hpos);
     }
     DEBUG(i_li_unparse());
}
