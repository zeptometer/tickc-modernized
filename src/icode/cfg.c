...
void i_bb_insertafter (i_bb_t b, icode_t pos, unsigned int len) {
     assert(0);
}

void i_bb_insertbefore (i_bb_t b, icode_t pos, unsigned int len) {
     assert(0);
}
...
static inline void markcall (icode_t cp) {
     growlist(icode_t, i_calls, i_calls_cur, i_calls_lim, dn_calls);
     i_calls[i_calls_cur++] = cp;
}

void i_fg_build (void) {
     unsigned i;
     icode_t cp = i_buf;	/* Start of code to analyze */
     icode_t lp = i_lim;	/* End of code to analyze */
     i_word_t op;		/* Current opcode */
