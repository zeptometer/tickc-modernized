...
#include <stdio.h>
#include <stdint.h>
#elif !defined(TICKC_TARGET_INTEGER_TYPES)
...
/* Icode cells hold either the 32-bit packed instruction or a target-sized
   immediate. This keeps pointers intact on LP64 while preserving the wire
   layout of the packed opcode fields in the low 32 bits. */
typedef uintptr_t i_word_t;
typedef i_word_t *icode_t;
...
extern i_local_t i_paramb (const struct v_aggregate *aggregate,
                           unsigned int n);
extern void i_resultb(const struct v_aggregate *aggregate);
enum { I_MAXPARAM=32 };		/* matches vcode MAX_ARGS */
