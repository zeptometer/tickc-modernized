 ...
 */
				/* Code */
icode_t i_buf, i_lim, i_ip;	/* Pointers to code (start, limit, current) */
...
static struct v_aggregate i_result_aggregate;
static int i_has_aggregate_result;

struct i_callb_immediate_record {
     struct v_aggregate aggregate;
     v_vptr callee;
};
...
i_local_t i_paramb(const struct v_aggregate *aggregate, unsigned int k) {
     i_local_t id;
     if (!aggregate || k >= max_param)
	  i_fatal("Invalid aggregate parameter");
     if (k >= i_params_cur) i_params_cur = k + 1;
     id = i_localb(aggregate->size);
     PARAMP(id) = 1;
     i_params[k].i = id;
     i_params[k].t = I_B;
     i_params[k].aggregate = *aggregate;
     return id;
}

void i_resultb(const struct v_aggregate *aggregate) {
     if (!aggregate) i_fatal("Invalid aggregate result");
     i_result_aggregate = *aggregate;
     i_has_aggregate_result = 1;
}

const struct v_aggregate *i_callb_immediate(
     v_vptr callee, const struct v_aggregate *aggregate) {
     struct i_callb_immediate_record *record;
     if (!callee || !aggregate) i_fatal("Invalid immediate aggregate call");
     NEW(record, 1);
     record->aggregate = *aggregate;
     record->callee = callee;
     return (const struct v_aggregate *)record;
}
...
void i_unparseinsn (FILE *fd, icode_t cp) {
     if (fd == 0) fd = stdout;
     fprintf(fd, "\t\t%p: ", cp);
     switch(get_op(cp)) {
     case i_op_nop:
...
union v_fp i_emit (unsigned int *offset, unsigned int *ninsn) {
     ...
     int aggregate_signature = i_has_aggregate_result;
     struct v_parameter parameters[I_MAXPARAM];
     v_reg_t parameter_values[I_MAXPARAM];
     unsigned int isz;		/* Size of code buffer: 1 icode -> 5 vcode */
     int sz;
     ...
     /* x86-64 sequences include REX prefixes and may materialize full-width
        immediates.  Leave enough room for the worst multi-instruction
        expansion as well as the fixed function prologue and epilogue. */
     sz = isz * 64 + 256;
     DEBUG(i_unparse());
     ...
     cp = (v_code *)v_code_alloc(i_cachesize+sz*sizeof(v_code));
     if (cp == NULL)
     ...
     for (i=0; i < i_params_cur; i++) {
	  ...
	  if (t == I_B) aggregate_signature = 1;
     }
				/* Initialize vcode */
     if (aggregate_signature) {
	  for (i = 0; i < i_params_cur; i++) {
	       parameters[i].type = i_params[i].t;
	       parameters[i].aggregate = i_params[i].t == I_B ?
		    &i_params[i].aggregate : NULL;
	  }
	  v_lambda_aggregate("",
	       i_has_aggregate_result ? &i_result_aggregate : NULL,
	       parameters, i_params_cur, parameter_values,
	       i_leafp ? V_LEAF : V_NLEAF, cp, sz * sizeof(v_code));
	  for (i = 0; i < i_params_cur; i++)
	       if (i_params[i].t == I_B)
		    ADDR(i_params[i].i) = v_localb(i_params[i].aggregate.size);
	       else {
		    ADDR(i_params[i].i) = v_local(i_params[i].t);
		    v_emit_store(i_params[i].t, parameter_values[i], v_lp,
				 ADDR(i_params[i].i), 0);
		    v_putreg(parameter_values[i], i_params[i].t);
		    SCLASS(i_params[i].i) = STACK;
	       }
     } else {
	  for (i = 0; i < i_params_cur; i++)
	       v_param_alloc(i, i_params[i].t, &ADDR(i_params[i].i));
	  v_clambda("", i_leafp ? V_LEAF : V_NLEAF, cp,
		      sz * sizeof(v_code));
     }
     ...
     if (aggregate_signature)
	  for (i = 0; i < i_params_cur; i++)
	       if (i_params[i].t == I_B) {
		    v_reg_t destination = v_local(V_P);
		    v_addpi(destination, v_lp, ADDR(i_params[i].i));
		    v_copyb(destination, parameter_values[i],
			    i_params[i].aggregate.size);
		    v_putreg(parameter_values[i], V_P);
	       }
