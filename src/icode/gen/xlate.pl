...
static inline void doisave (unsigned int l, bvt bv, void *v) {
     unsigned int rf = (unsigned int)(uintptr_t)v;
...
static void callinit (icode_t cp) {
    ...
    do {
	 ...
	 case i_op_callbb:	case i_op_callbbi:
	      if (num_i) {
		   isav = (i_ralloctype == RA_LS) ? 
			lscsav(INT, cbb->scn) : get_isav(ccp);
		   if (isav) {
		       bv_eachbit(isav, doisave, (void *)(uintptr_t)RESTORE);
		       bv_eachbit(bv_rdiff(iargs, isav), 
				  doisave, (void *)(uintptr_t)NORESTORE);
		   } else {
		       bv_eachbit(iargs, doisave,
			  (void *)(uintptr_t)NORESTORE);
		   }
	      }
	      if (num_f) {
		   ...
		   if (fsav) {
		       bv_eachbit(fsav, dofsave, (void *)(uintptr_t)RESTORE);
		       bv_eachbit(bv_rdiff(fargs, fsav), 
				  dofsave, (void *)(uintptr_t)NORESTORE);
		   } else {
		       bv_eachbit(fargs, dofsave,
			  (void *)(uintptr_t)NORESTORE);
		   }
	      }
	      v_push_init(&vcs);
...
static icode_t i_x_retv(icode_t cp) {
     v_retv();
     return cp+i_isize;
}
icode_t i_x_callv (icode_t cp) {
...
static icode_t i_x_callvi (icode_t cp) {
     if (!vpushinit) { callinit(cp); }
...
static icode_t i_x_jp (icode_t cp) {
...
static icode_t i_x_jpi (icode_t cp) {
     v_jv(get_imm(cp));
     return cp+i_isize; 
}
static icode_t i_x_lbl (icode_t cp) {
     i_local_t x0 = get_rd(cp);		/* Dummy locals */
     v_label(x0);
     return cp+i_isize; 
}

static icode_t codeptr;
static icode_t i_x_self (icode_t cp) {
     i_local_t x0 = get_rd(cp);		/* Dummy locals */
     v_reg_type tr0;		/* Dummy registers */
     assign2(tr0, i_find, t0, x0);
     v_setp(tr0, (uintptr_t)codeptr);
     i_store(tr0, x0, I_P);
     return cp+i_isize;
}
static icode_t i_x_refmul (icode_t cp) { return cp+i_isize; }
static icode_t i_x_refdiv (icode_t cp) { return cp+i_isize; }
static icode_t i_x_nop (icode_t cp) { return cp+i_isize; }
static icode_t i_x_unspecified (icode_t cp) {
...
static icode_t i_x_undefined (icode_t cp) {
     fprintf(stderr, 
...
SW: {
    ...
    /f_argb/ && do {
	print <<"EOF"
static icode_t i_x_$op (icode_t cp) {
     v_reg_type address;
     if (!vpushinit) { callinit(cp); vpushinit = 1; }
     assign3(address, i_load, t0, get_rd(cp), I_P);
     v_arg_pushb(&vcs, address,
		 (const struct v_aggregate *)get_imm(cp));
     return cp+i_isize;
}
EOF
    ;
...
static icode_t i_x_$op (icode_t cp) {
     v_reg_type address;
     assign3(address, i_load, t0, get_rd(cp), I_P);
     v_retb(address, (const struct v_aggregate *)get_imm(cp));
     return cp+i_isize;
}
EOF
    ;
	...
	print <<"EOF"
static icode_t i_x_$op (icode_t cp) {
     v_reg_type destination, source;
     assign3(destination, i_load, t0, get_rd(cp), I_P);
     assign3(source, i_load, t1, get_rs(cp), I_P);
     v_copyb(destination, source, (unsigned int)get_imm(cp));
     return cp+i_isize;
}
EOF
    ;
	...
	print <<"EOF"
static icode_t i_x_$op (icode_t cp) {
     const struct i_callb_immediate_record *record =
	  (const struct i_callb_immediate_record *)get_imm(cp);
     v_reg_type destination, returned;
     if (!vpushinit) { callinit(cp); }
     assign3(destination, i_load, t0, get_rd(cp), I_P);
     returned = v_ccallb(&vcs, record->callee, &record->aggregate);
     v_copyb(destination, returned, record->aggregate.size);
     v_putreg(returned, V_P);
     dorestore(); vpushinit = 0;
     return cp+2*i_isize;
}
EOF
    ;
...
static icode_t i_x_$op (icode_t cp) {
     const struct v_aggregate *aggregate =
	  (const struct v_aggregate *)get_imm(cp);
     v_reg_type destination, callee, returned;
     if (!vpushinit) { callinit(cp); }
     assign3(destination, i_load, t0, get_rd(cp), I_P);
     assign3(callee, i_load, t1, get_rs(cp), I_P);
     returned = v_rccallb(&vcs, callee, aggregate);
     v_copyb(destination, returned, aggregate->size);
     v_putreg(returned, V_P);
     dorestore(); vpushinit = 0;
     return cp+2*i_isize;
}
...
static icode_t i_x_$op (icode_t cp) {
     i_local_t x0 = get_rd(cp), x1;	/* Dummy locals */
     intptr_t x2;			/* Target-sized displacement */
     v_reg_type tr0, tr1;	/* Dummy registers */
     x1 = get_rs(cp);
     x2 = get_imm(cp);
     assign2(tr0, i_find, ${f}t0, x0);
...
EOF
	...
	print <<"EOF"
static icode_t i_x_$op (icode_t cp) {
     i_local_t x0 = get_rd(cp), x2;	/* Dummy locals */
     intptr_t x1;			/* Target-sized displacement */
     v_reg_type tr0, tr1;	/* Dummy registers */
     x1 = get_imm(cp);
     x2 = get_rs(cp);
