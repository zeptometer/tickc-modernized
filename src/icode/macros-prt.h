...
#define f_dargb(cp, name) do { fprintf(fd, name "(r%d, %p)\n", \
	get_rd(cp), (void *)get_imm(cp)); } while (0)
#define f_dretb(cp, name) f_dargb(cp, name)
#define f_dcallb(cp, name) do { fprintf(fd, name "(r%d, r%d, %p)\n", \
	get_rd(cp), get_rs(cp), (void *)get_imm(cp)); } while (0)
#define f_dcallbi(cp, name) do { fprintf(fd, name "(r%d, %p)\n", \
	get_rd(cp), (void *)get_imm(cp)); } while (0)
#define f_dblkcopy(cp, name) do { fprintf(fd, name "(r%d, r%d, %lu)\n", \
	get_rd(cp), get_rs(cp), (unsigned long)get_imm(cp)); } while (0)
