...
/*
   Icode instructions have two target-sized cells. The first cell uses
   the low 32 bits for packed opcode/register fields. The second cell is
   either another packed register field or a target-sized immediate.
...
#define set_imm(_im) ((i_word_t)(uintptr_t)(_im))
#define get_imm(_is) ((i_word_t)(*(_is+1)))
...
#define f_argb(op, arg, descriptor) do {				\
     *i_ip++ = set_op(op)|set_rd(arg);				\
     *i_ip++ = set_imm(descriptor);				\
     REFCNT(arg) += i_refc;					\
} while (0)

#define f_retb(op, val, descriptor) do {				\
     *i_ip++ = set_op(op)|set_rd(val);				\
     *i_ip++ = set_imm(descriptor);				\
     REFCNT(val) += i_refc; ++i_nbb;				\
} while (0)

#define f_callb(op, result, callee, descriptor) do {		\
     *i_ip = set_op(op)|set_rd(result)|set_rs(callee);		\
     *(i_ip+1) = set_imm(descriptor);				\
     *(i_ip+i_isize) = 0; *(i_ip+i_isize+1) = 0;		\
     i_ip += 2*i_isize; ++i_nbb; i_leafp = 0;			\
     REFCNT(result) += i_refc; REFCNT(callee) += i_refc;	\
} while (0)

extern const struct v_aggregate *i_callb_immediate(v_vptr callee,
					 const struct v_aggregate *aggregate);
#define f_callbi(op, result, callee, descriptor) do {		\
     *i_ip = set_op(op)|set_rd(result);				\
     *(i_ip+1) = set_imm(i_callb_immediate((v_vptr)(callee), descriptor)); \
     *(i_ip+i_isize) = 0; *(i_ip+i_isize+1) = 0;		\
     i_ip += 2*i_isize; ++i_nbb; i_leafp = 0;			\
     REFCNT(result) += i_refc;					\
} while (0)

#define f_blkcopy(op, dst, src, size) do {			\
