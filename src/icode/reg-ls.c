...
static inline unsigned int lrexpire (i_ll_t l, i_lle_t e, 
				     void *v1, void *sc) {
     i_cnt_t pos = (i_cnt_t)(uintptr_t)v1;
     i_local_t loc;
     if (e->lr->beg <= pos)
     ...
     i_reg_put((unsigned int)(uintptr_t)sc, ADDR(loc));
...
static void ra_ls_internal (int fp, i_lr_t lrbeg, i_lr_t lrlim) {
     ...
     for (lr = lrlim; lr  >= lrbeg; lr--) {
	  ...
	  i_llsbwd(active, lrexpire, (void *)(uintptr_t)lr->end,
		    (void *)(uintptr_t)fp);
	  ...
	  if (i_llisfull(active)) {
	       ...
	       if (use_refc) {
		    ...
		    i_llsfwd(active, lrstartsfirst,
			      (void *)(uintptr_t)lr->end, NULL);
	  ...
	  if (r != -1) {
	       ...
	       le = i_llsbwd(active, lrstartsbefore,
			  (void *)(uintptr_t)lr->beg, NULL);
