typedef struct {
     ...
     char *_isinstruction;
     char **_ntname;
     int (*isimmediate) ARGS((Node));
     void (*emit2) ARGS((Node));
     void (*emit3) ARGS((Node));
...
typedef struct {
     void (*genclosurebeg) ARGS((Code));
     void (*genclosureend) ARGS((Code));
     void (*emitclosurebeg) ARGS((Code));
     void (*emitclosureend) ARGS((Code));
     void (*doregalloc) ARGS((short,int));
#if 0
     int uniqueid;
#endif
} Tinterface;
