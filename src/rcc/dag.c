...
Node listnodes(tp, tlab, flab) Tree tp; int tlab, flab; {
     ...
     case CALL:  { 
	  ...
	  assert(tlab == 0 && flab == 0);
	  if (tp->op == CALL+B &&
	      !(eval.ticklevel ? dIR : IR)->wants_callb) {
     ...
     case LT:    {
	  ...
	  if (tlab)
	       assert(flab == 0),
		    p = newnode(tp->op, l, r, findlabel(tlab));
	  else if (flab) {
	       ...
	       p = newnode(op, l, r, findlabel(flab));
	  }
	  if (p) {
	       p->type = tp->kids[0]->type;
	       list(p);
	  }
     ...
     case ASGN:  { 
	  ...
	  {
	       Node asgn = newnode(tp->op, l, r, NULL);
	       asgn->type = tp->kids[1]->type;
	       list(asgn);
	  }
     ...
     case VAARG: case VASTART: case VAEND:
	  if (eval.ticklevel)
	       error("va_start/va_arg/va_end inside a Tick-C quotation is not "
		     "supported; use the direct-vcode variadic API\n");
	  else
	       assert(cbe.have);
	  break;
     default:
