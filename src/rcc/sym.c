...
Symbol constant(ty, v) Type ty; Value v; {
     ...
     ty = unqual(ty);
     if (ty == longtype)
	  h = ((unsigned long)v.l ^ ((unsigned long)v.l >> 16))&(HASHSIZE-1);
     else if (ty == unsignedlong)
	  h = (v.ul ^ (v.ul >> 16))&(HASHSIZE-1);
     else
	  h = v.u&(HASHSIZE-1);
     for (p = constants->buckets[h]; p; p = p->link)
	  if (eqtype(ty, p->sym.type, 1))
	       ...
	       case INT:
		    if (ty == longtype ? equalp(l) : equalp(i))
			 return &p->sym;
		    break;
	       case UNSIGNED:
		    if (ty == unsignedlong ? equalp(ul) : equalp(u))
			 return &p->sym;
		    break;
...
char *vtoa(ty, v) Type ty; Value v; {
     ...
     case INT:
	  if (ty == longtype)
	       return stringf("%ld", v.l);
	  return stringd(v.i);
     case UNSIGNED:
	  if (ty == unsignedlong)
	       return stringf("%lu", v.ul);
	  if ((v.u&~0x7fff) == 0)
	       return stringd(v.u);
	  else
