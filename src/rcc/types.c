...
#undef xx
     ...
     assert(unsignedlong->size >= voidptype->size);
     assert(longtype->size >= voidptype->size);
}
...
char *tfVcodeSuffix(t) Type t; {
     ...
     if (t == unsignedlong)
	  return "ul";
     switch (t->op) {
     case CHAR: case SHORT: case INT: case ENUM:
	  return "i";
     case UNSIGNED:
	  return "u";
     ...
     case VOID:
	  return "v";
     case CSPEC: case VSPEC:
	  return tfVcodeSuffix(t->type);
     default:
	  assert(0);
     }
     return "i";
}
char *tfVcodeType(t) Type t; {
     t = unqual(t);
     if (t == longtype)
	  return "I_L";
     if (t == unsignedlong)
	  return "I_UL";
     return tfVerbose(t->op);
}

static void tfAggregateFields(Type ty, int base, char **text, int *count) {
     Field field;
     int i, elements;

     ty = unqual(ty);
     if (*count >= 16) {
	  error("aggregate ABI supports at most 16 scalar fields\n");
	  return;
     }
     if (isstruct(ty)) {
	  for (field = ty->u.sym->u.s.flist; field; field = field->link) {
	       if (field->bitsize) {
		    *text = stringf("%s,%d,I_U", *text,
				    base + field->offset);
		    ++*count;
	       } else
		    tfAggregateFields(field->type, base + field->offset,
				      text, count);
	  }
	  return;
     }
     if (isarray(ty)) {
	  if (!ty->type || ty->type->size <= 0) {
	       error("incomplete array in aggregate ABI type\n");
	       return;
	  }
	  elements = ty->size / ty->type->size;
	  for (i = 0; i < elements; i++)
	       tfAggregateFields(ty->type, base + i * ty->type->size,
				 text, count);
	  return;
     }
     if (!isscalar(ty)) {
	  error("unsupported aggregate ABI field type `%t'\n", ty);
	  return;
     }
     *text = stringf("%s,%d,%s", *text, base, tfVcodeType(ty));
     ++*count;
}

/* Return a generated-C expression that constructs a transient ABI layout.
   The runtime copies the descriptor before the expression's arena storage is
   reclaimed. */
char *tfAggregate(Type ty) {
     char *fields = string("");
     int count = 0;

     ty = unqual(ty);
     assert(isstruct(ty));
     tfAggregateFields(ty, 0, &fields, &count);
     return stringf("_tc_aggregate(%d,%d,%d%s)", ty->size, ty->align,
		    count, fields);
}
