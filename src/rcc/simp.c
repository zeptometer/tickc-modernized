...
#define foldrtc(TYPE,VAR,FMT,OP,RTYPE) \
{\
  char *lname = NULL, *rname = NULL;\
  if (l->op == RTC+TYPE \
      && (r->op == CNST+TYPE || r->op == RTC+TYPE)) {\
    s = l->u.sym; lname = s->x.name;\
    rname = (r->op == CNST+TYPE) ? stringf(#FMT,r->u.v.VAR) \
				 : r->u.sym->x.name; \
  } else if (r->op == RTC+TYPE && l->op == CNST+TYPE) {\
    lname = stringf(#FMT,l->u.v.VAR); \
    s = r->u.sym; rname = s->x.name;\
  } \
  if (rname) {\
    assert(lname); assert(s); \
    p = tree(RTC+ttob(RTYPE), RTYPE, NULL, NULL); \
    s->x.name = stringf("%s" #OP "%s", lname, rname); \
    s->type = RTYPE; \
    p->u.sym = s; \
    return p; \
  }\
}
#define ufoldrtc(TYPE,OP,RTYPE) \
{\
  char *lname;\
  if (l->op == RTC+TYPE) {\
    s = l->u.sym; lname = s->x.name;\
    p = tree(RTC+ttob(RTYPE), RTYPE, NULL, NULL); \
    s->x.name = stringf(#OP "%s", lname); \
    s->type = RTYPE; \
    p->u.sym = s; \
    return p; \
  }\
}
#define commutertc(L,R) \
{\
  if (generic(R->op) == RTC && generic(L->op) != RTC) {\
    Tree t = L; L = R; R = t;\
  }\
}
...
int needconst;
static Tree simplifyrtc ARGS((int, Type, Tree, Tree));
...
static Tree simplifyrtc (op, ty, l, r) int op; Type ty; Tree l, r; {
     Tree p;
     Symbol s = NULL;

     switch (op) {
     case ADD+D:
	  foldrtc(D,d,%e,+,doubletype);
	  commutertc(r,l);
	  break;
     case ADD+F:
	  foldrtc(F,f,%f,+,floattype);
	  commutertc(r,l);
	  break;
     case ADD+I:
	  foldrtc(I,i,%d,+,inttype);
	  commutertc(r,l);
	  break;
     case ADD+P:
	  foldrtc(P,p,%p,+,voidptype);
	  commutertc(r,l);
	  break;
     case ADD+U:
	  foldrtc(U,u,%u,+,unsignedtype);
	  commutertc(r,l);
	  break;
     case AND+I:
	  foldrtc(I,i,%d,&&,inttype);
     ...
     case DIV+U:
	  ...
	  break;
     case EQ+D:
	  foldrtc(D,d,%e,==,inttype);
	  commutertc(r,l);
	  break;
     case EQ+F:
	  foldrtc(F,f,%f,==,inttype);
	  commutertc(r,l);
	  break;
     case EQ+I:
	  foldrtc(I,i,%d,==,inttype);
     ...
     case NE+U:
	  ...
	  break;
     case NEG+D:
	  ufoldrtc(D,-,doubletype);
	  break;
     case NEG+F:
	  ufoldrtc(F,-,floattype);
	  break;
     case NEG+I:
	  ufoldrtc(I,-,inttype);
	  break;
...
Tree simplify(op, ty, l, r) int op; Type ty; Tree l, r; {
     ...
     /* The original constant folder stores I/U values in 32-bit union
	fields.  Preserve LP64 operations until their folding rules are made
	width-aware; folding them as int silently truncates long constants. */
     if (ty == longtype || ty == unsignedlong)
	  switch (generic(op)) {
	  case ADD: case BAND: case BCOM: case BOR: case BXOR:
	  case DIV: case LSH: case MOD: case MUL: case NEG:
	  case RSH: case SUB:
	       return tree(op, ty, l, r);
	  }

     /* fold runtime constants */
     if (l && r && (generic(l->op) == RTC || generic(r->op) == RTC))
	  return simplifyrtc(op, ty, l, r);
	...
	case CVI+I:
	case CVU+U:
	  /* LP64 requires explicit int/long and unsigned/unsigned-long width
	     conversions even though they share the historical I/U opcode class. */
	  break;
     ...
     case MUL+I:
	  ...
	  if (l->op == CNST+I && r->op == ADD+I && r->kids[1]->op == CNST+I)
	       /* c1*(x + c2) => c1*x + c1*c2 */
	       return simplify(ADD+I, ty, simplify(MUL+I, ty, l, r->kids[0]),
			       simplify(MUL+I, ty, l, r->kids[1]));
	  if (l->op == CNST+I && r->op == SUB+I && r->kids[1]->op == CNST+I)
	       /* c1*(x - c2) => c1*x - c1*c2 */
	       return simplify(SUB+I, ty, simplify(MUL+I, ty, l, r->kids[0]),
			       simplify(MUL+I, ty, l, r->kids[1]));
	  if (l->op == CNST+I && l->u.v.i > 0 && (n = ispow2(l->u.v.i)) != 0)
	       /* 2^n * r => r<<n */
	       return simplify(LSH+I, ty, r, consttree(n, inttype));
...
/*	 && isspec(p->type)) {*/
				/* If computing address from a spec,
				   do not introduce a new local with that 
				   address, but explicitly compute the sum;
				   statically, we can't know the address. */

				/* Create a symbol to point to new address */
	  Symbol q = install(mkname(NAMELCL), &identifiers, level, FUNC);
	  q->type = ty;
	  q->sclass = AUTO;
	  addlocal(q);

	  ++eval.unqtlevel;	/* Change eval.unqtlevel so that tree() does
				   not try to quote the spec p */
#if 0
	  e = idtree(p);
	  e->op = generic(e->op) + P;
#endif
	  e = tree(ADDP, ty, e, consttree(n, unsignedtype));
	  --eval.unqtlevel;
	  walk(asgntree(ASGN, idtree(q), e), 0, 0);
	  e = idtree(q);
     } else if (cbe.have && (cbe.emit || eval.rtcp || eval.unqtlevel)) {
				/* Return explicit ADD tree rather than
				   simplified address if we're compiling to C
				   and we're either in vanilla code or in
				   unquoted part of dynamic code (other
				   dynamic code should be handled above) */
#if 0
	  /* 
	     this is rendered obsolete by the more general check when printing
	     out ADDRG/ADDRL/ADDRF in unparsetree()
	  */
	  if (isarray(p->type))
	       e = rvalue(e);
#endif
