...
static Tree aggregateexpr ARGS((Type));

static void aggregatevalues(Type ty, int base, unsigned offsets[],
			    unsigned types[], int *count) {
     Field field;
     int i, elements;
     ty = unqual(ty);
     if (isstruct(ty)) {
	  for (field = ty->u.sym->u.s.flist; field; field = field->link)
	       if (field->bitsize) {
		    if (*count >= 16) break;
		    offsets[*count] = base + field->offset;
		    types[(*count)++] = ttov(unsignedtype);
	       } else
		    aggregatevalues(field->type, base + field->offset,
				    offsets, types, count);
	  return;
     }
     if (isarray(ty)) {
	  elements = ty->type && ty->type->size ? ty->size / ty->type->size : 0;
	  for (i = 0; i < elements; i++)
	       aggregatevalues(ty->type, base + i * ty->type->size,
			       offsets, types, count);
	  return;
     }
     if (*count >= 16) return;
...
static Tree aggregateexpr(Type ty) {
     unsigned offsets[16], types[16];
     int count = 0, i;
     Tree args = NULL, f;
     Symbol function;

     ty = unqual(ty);
     aggregatevalues(ty, 0, offsets, types, &count);
     args = tree(ARGI, unsignedtype, consttree(ty->size, unsignedtype), args);
     args = tree(ARGI, unsignedtype, consttree(ty->align, unsignedtype), args);
     args = tree(ARGI, unsignedtype, consttree(count, unsignedtype), args);
     for (i = 0; i < count; i++) {
	  args = tree(ARGI, unsignedtype,
		      consttree(offsets[i], unsignedtype), args);
	  args = tree(ARGI, unsignedtype,
		      consttree(types[i], unsignedtype), args);
     }
     function = findtcsym(string("_tc_aggregate"), globals);
     f = idtree(function);
     return calltree(f, freturn(function->type), args, NULL);
}
...
Tree unary() {
     ...
     case TPARAM:     {
	  ...
	  f = idtree(findtcsym(eval.fc ? tcname.Paramf : tcname.Param, 
			       globals));
	  descriptor = isstruct(ty) ? aggregateexpr(ty) :
	       cast(consttree(0, unsignedtype), voidptype);
	  args = tree(ARGP, descriptor->type, descriptor,
		      tree(ARGI, p->type, p,
     ...
     break;
     case TCOMPILE:      {
	  Tree f, typearg, descriptor, args;
	  ...
	  f = idtree(findtcsym(eval.fc ? tcname.Compilef : tcname.Compile, 
			       globals));
	  typearg = consttree(ttov(ty), unsignedtype);
	  descriptor = isstruct(ty) ? aggregateexpr(ty) :
	       cast(consttree(0, unsignedtype), voidptype);
	  args = tree(ARGP, descriptor->type, descriptor,
		      tree(ARGI, unsignedtype, typearg, args));
	  p = calltree(f, ptr(func(ty, NULL, NULL, 1)), args, NULL);

	  if (cbe.have) {
	       cbufflushon();
	       cbufflush(stringf("%s,%s", unparsetree(typearg),
				 unparsetree(descriptor)));
	       cbufputc(')');
	       cbufputc(t);
	  }
     }
...
Tree idtree(p) Symbol p; {
	 ...
	 ||  p->sclass == STATIC || p->sclass == EXTERN)
	  ...
	  if (isstruct(p->type) &&
	      !(eval.ticklevel ? dIR : IR)->wants_argb) {
...
Type binary(xty, yty) Type xty, yty; {
     xty = promote(unqual(xty));
     yty = promote(unqual(yty));
     ...
     if (xty == longtype || yty == longtype) {
	  if ((xty == unsignedtype || yty == unsignedtype)
	      && unsignedtype->size >= longtype->size)
	       return unsignedlong;
	  return longtype;
     }
     if (xty == unsignedtype || yty == unsignedtype)
...
Tree cast(p, type) Tree p; Type type; {
     ...
     case INT:     p = retype(p, pty->size > inttype->size ? longtype :
	                         inttype);
	                                                     break;
     case DOUBLE:  p = retype(p, doubletype);              break;
     case ENUM:    p = retype(p, inttype);                 break;
     case UNSIGNED:p = retype(p, pty->size > unsignedtype->size ?
	                         unsignedlong : unsignedtype);  break;
     case POINTER:
	  if (isptr(ty)) {
	       Field q;
	       ...
	       p = simplify(CVP,
			    ty->op == UNSIGNED && ty->size > unsignedtype->size ?
			    unsignedlong : unsignedtype, p, NULL);
	  break;
     case VSPEC:
	  if (! isvspec(type))
     ...
     }
     {
	  ...
	  if (pty != sty)
	       ...
	       else if (pty == unsignedtype || pty == unsignedlong)
		    if (sty == doubletype) {
...
static Type super(ty) Type ty; {
	if (ty->op == INT && ty->size > inttype->size)
	  return longtype;
	if (ty->op == UNSIGNED && ty->size > unsignedtype->size)
	  return unsignedlong;
