#if defined(__TCC__) || defined(__STRICT_ANSI__)
#define inline
#endif
...
#define LEFTKID(p) ((p)->kids[0])
#define RIGHTKID(p) ((p)->kids[1])
...
#define isrtcop(op) (generic(op) == RTC)

#define isrvalue(t) (generic((t)->op) == INDIR)
#define iscsval(n)  ((n) && generic((n)->op) == ICS)
...
#define iscspec(t)    (unqual(t)->op == CSPEC)
#define isvspec(t)    (unqual(t)->op == VSPEC)
#define isspec(t)     (iscspec(t) || isvspec(t))
#define isindirect(t) (isptr(t) || iscspec(t) || isvspec(t))
...
#define isscalar(t)   (unqual(t)->op <= POINTER \
                    || unqual(t)->op == ENUM \
     		    || unqual(t)->op == CSPEC || unqual(t)->op == VSPEC)
...
typedef struct localsList * LocalsList;
...
void (*iLocal) ARGS((Symbol)); /* Upper case to not conflict with `C local() */
...
extern Interface *IR;
extern Interface *sIR;
extern Interface *dIR;
...
/* Enumeration which defines Closure Entries: 
   LoCaL (to ` expression), Free VSpec, Free CSpec, Tick-expr Free Variable, 
   Run-Time Constant */
typedef enum { CELCL=0, CEFVS, CEFCS, CETFV, CERTC } ceclass;
typedef enum { NAMERTC=0, NAMESPEC, NAMEFV, NAMELCL} nameclass;
#ifdef __TCC__
#define ceclass unsigned
#define nameclass unsigned
#endif
enum {
...
enum { TEXPR=44<<4,		/* TEXPR is only used in the front end */
       	TEXPRB=TEXPR+B,
       	TEXPRC=TEXPR+C,
       	TEXPRD=TEXPR+D,
       	TEXPRF=TEXPR+F,
       ...
       RTC=46<<4,		/* Run-Time Constant */
       	...
       	RTCS=RTC+S,
       	RTCU=RTC+U,
       CRET=47<<4,		/* Closure register return pseudo-op */
       	CRETB=CRET+B,
       	CRETC=CRET+C,
       	CRETD=CRET+D,
       	CRETF=CRET+F,
       	...
       	CRETS=CRET+S,
       	CRETU=CRET+U,
       ADRFF=48<<4,		/* Addresses of `-expression free variable */
       	ADRFFP=ADRFF+P,
       ADRFL=49<<4,
       	ADRFLP=ADRFL+P
};
...
/* Kxxx: tests, jumps, and labels based on run-time constants */
enum {
	KCOND=53<<4,
	KLABEL=54<<4,
	 KLABELV=KLABEL+V,
...
struct code {
     ...
     union {
	  struct {
	       ...
	       Type type;
	       Symbol cgf;
	       List labels;	/* local labels */
	       List locals;	/* local variables */
	       List fakeregs;	/* pseudo-regs assigned to spilled locals */
	       Table lcl;	/* locals to be stored in closure */
	       Table fvs;	/* free vspecs */
	       Table fcs;	/* free cspecs */
	       Table tfv;	/* other free vars */
	       Table rtc;	/* runtime constants */
	       Table statics;	/* static inside ` */
	       
	       int hascall;
	       int hasaggregate;
	       unsigned int locreg[2];
	       unsigned int regmask[2];
	  } closure;
	  struct {
	       int level;
	       Symbol *locals;
...
struct symbol {
	...
	unsigned nfvp:1;	/* Discovered this is not a free variable */
        unsigned tfvp:1;	/* Free variable */
        unsigned lclp:1;	/* Local */
        unsigned rtcp:1;	/* Run-time constant */
	unsigned nrtcp:1;	/* Discovered this is not run-time constant */
        unsigned drtcp:1;	/* Derived run-time constant */
        unsigned specp:1;	/* Spec */
	...
	union {
		...
		union {
		  	Tree t;
			char *s;
		} x;  /* 'x' field, used for runtime constants, specs, tfvs */
	} u;
        Symbol original;
	Symbol copy;
	Xsymbol x;
...
struct tree {
	int op;
        int scope;		/* used to determine when an unquoted object
				   needs to be stored in a cspec's closure */
	Type type;
	Tree kids[2];
	Node node;
	union {
...
struct type {
	...
	Ytype y;	/* omit */
#endif			/* omit */
        int fullp;		/* `C-C: use unparsefulltype() iff fullp=1  */
};
struct table {
	int level;
	Table previous;
...
extern const int outfdIdx;
extern const int errfdIdx;
extern const int dcgfdIdx;
extern const int cccfdIdx;
extern const int hhhfdIdx;
extern const int sssfdIdx;
extern void sw2dasm ARGS((void));
extern void sw2sasm ARGS((void));
...
extern struct code scodehead;
extern Code scodelist;
extern struct code dcodehead;
extern Code dcodelist;
...
extern Type intvspectype;
extern Type voidcspectype;
extern Type valisttype;
extern Type type ARGS((int, Type, int, int, void *));
...
extern Tree notetree ARGS((char *));
...
extern Tree ktest ARGS((Tree));
extern Tree expr0 ARGS((int));
extern Tree expr ARGS((int));
extern Tree expr1 ARGS((int));
extern Tree unary ARGS((void));
extern Tree tickexpr ARGS((void));
...
extern void outsfdx ARGS((char *, int));

extern void print ARGS((char *, ...));
extern void outsverbatim ARGS((char *));
...
extern void outflushfdx ARGS((int));
extern void outclearfdx ARGS((int));
...
extern char *mkname ARGS((nameclass));
...
extern void sw2scode ARGS((void));
extern void sw2dcode ARGS((void));
...
extern Node kjump ARGS((int));
extern void retcode ARGS((Tree));
extern void statement ARGS((int, Swtch, int, int));
...
extern Symbol kfindlabel ARGS((int));
extern Symbol findtype ARGS((Type));
extern void forall ARGS((Table, void (*)(Symbol, void *), void *));
...
extern Symbol installcopy ARGS((Symbol, Table*, int, int));
...
extern Table permtable ARGS((Table, int));
extern Symbol temporary ARGS((int, Type, int));
extern char *vtoa ARGS((Type, Value));
extern Symbol annotation ARGS((char *)); 
extern Symbol findtcsym ARGS((char *, Table));
...
extern char *unparsetree ARGS((Tree));
...
extern void tickfixtree ARGS((int, Tree *, Tree *));
...
extern char *typestring ARGS((int, Type ty, char *id));
...
extern int ttov ARGS((Type));
extern void typeInit ARGS((void));
extern int variadic ARGS((Type));

extern char tfDense ARGS((Type));
extern char tfSparse ARGS((Type));
extern char tfCaps ARGS((Type));
extern char* tfVerbose ARGS((int));
extern char* tfVcodeSuffix ARGS((Type));
extern char* tfVcodeType ARGS((Type));
extern char* tfAggregate ARGS((Type));
...
extern void *calloc ARGS((unsigned, unsigned));
...
extern void *memcpy ARGS((void *, void *, unsigned));
...
extern unsigned strlen ARGS((const char *));
extern int strncmp ARGS((const char *, const char *, unsigned));
extern char *strncpy ARGS((char *, const char *, unsigned));
extern char *strcat ARGS((char *, char *));
extern int write ARGS((int, char *, int));

extern Symbol dclglobal ARGS((int, char *, Type, Coordinate *, LocalsList));
extern Symbol dcllocal ARGS((int, char *, Type, Coordinate *, LocalsList));
extern void computefieldoffsets ARGS((Type));

extern void tickrefcnt ARGS((int, int));
extern Tree calllocaltree ARGS((Type, int));
extern Tree ticktree ARGS((Symbol, Type));
extern void rtctree ARGS((Tree *));
extern Type mkcspec ARGS((Type));
extern Type mkvspec ARGS((Type));
extern Type derefcvspec ARGS((Type));
extern void closureret ARGS((Tree));

extern char *mangle ARGS((char *));

extern Symbol closureaddsym ARGS((Symbol, ceclass));
extern Symbol closureaddtree ARGS((Tree, ceclass));
extern Tree closurefield ARGS((Symbol));
extern Type declclosure ARGS((Type));
extern Type declnamedclosure ARGS((Symbol, Symbol, Symbol));
extern Symbol defclosure ARGS((Type));

extern void initevalinfo ARGS((void));
extern void storeevalinfo ARGS((Code, Symbol, Type));

extern void cbufput ARGS((char *));
extern void cbufputc ARGS((char));
extern void cbufflush ARGS((char *));
extern char * cbufstring ARGS((void));
extern void cbufputverbatim ARGS((char *));
extern void cbufclear ARGS((void));
extern void cbufflushon ARGS((void));
extern void cbufflushoff ARGS((void));
extern void cbacksuspend ARGS((void));
extern void cbackrestart ARGS((void));

extern int idef ARGS((char *, int));
extern int idefScalarFamily ARGS((char *, int, int));
extern int idefScalarConversion ARGS((char *, char *, int, int));
extern void itabinit ARGS((void));
extern void itabdump ARGS((void));

struct tcnamelist {		/* Names for interfacing with run-time sys */
     char *Cgfroot;
     char *Cgfsuffix;
     char *Cgffield;
     char *Labfield;
...
extern struct tcnamelist tcname;
...
extern struct cbestate cbe;

struct evalstate {
     Code currentclosure;/* current closure being processed */
     Symbol cgfsym;	/* current code generating function */
     ...
     Table tfv;		/* other free vars */
     Table rtc;		/* runtime constants */
     Table targ;	/* dynamic jump targets */

     Table luse;	/* labels used by gotos in current cspec */
     Table ldef;	/* labels defined in current cspec */

     Table statics;	/* variables with static storage class */
     List labels;	/* labels emitted in the current `-expr */

     Table esymaddr;	/* external syms (addresses); spans `-exprs (includes
			   func names, strings) */
     Table esymcnst;	/* external syms (constants) */
     Table esymstr;	/* external syms (strings) */
     Table esymval;	/* external syms (values); span `-exprs (include
			   constants, and global and static scalars) */

     int ticklevel;	/* scope enclosing `-expr; 0 iff not parsing `-expr */
     int unqtlevel;	/* depth of unquoted expression */
     int lcllevel;	/* 1 when walking dynamic code for CELCL; else 0 */

     unsigned autounroll:1; /* try to automatically dynamically unroll loops */

     unsigned fc:1;	/* true if doing fast (vcode) compilation */
     unsigned leafp:1;	/* true if cspec does not contain a function call */
     unsigned tickp:1;	/* true if parsing `-expr or closure assignment */
     unsigned rtcp:1;	/* true if parsing operand of $ */

     unsigned tExists:1;	/* true if `-expr exists in translation unit */
     unsigned tExistsHere:1;	/* true iff `-expr exists in this function */
     unsigned tEmitting:1;	/* true if a template is being emitted */
     unsigned tCompiling:1;	/* true if compiling vcode templates */
};

extern struct evalstate eval;
