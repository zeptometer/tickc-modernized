...
char *unparsetree(Tree t) {
     ...
     case CNST:
	  ...
	  case I: return t->type == longtype ? stringf("%ld", t->u.v.l) :
		       stringf("%d", t->u.v.i);
	  case P: return stringf("%x", t->u.v.p);
	  case S: return stringf("%d", t->u.v.us);
	  case U: return t->type == unsignedlong ? stringf("%lu", t->u.v.ul) :
		       stringf("%u", t->u.v.u);
	  default: assert(0);
	  }
     case ARG:
	  ...
	  if (generic(((k = t->kids[0]))->op) == ADDRL
	      || generic(k->op) == ADDRF || generic(k->op) == ADDRG) {
	       ...
	       if (isarray(k->u.sym->type))
		    ...
		    return stringf("%s((unsigned long)%s)%s", pre,
			   k->u.sym->name,
				   post);
	       else
     ...
     case ADDRG: case ADDRF: case ADDRL:
	  ...
	  ty = t->u.sym->type;
	  if (isarray(ty))
	       return stringf("((unsigned long) %s)", t->u.sym->name);
	  else if (isfunc(ty))
