...
static void     tprelabel       ARGS((Node));
static Node*    prune           ARGS((Node, Node*));
static void     putreg          ARGS((Symbol, Node));
...
static char NeedsReg[] = {
	...
	0, 0,                   /* JUMP LABEL (37) */
	0, 0, 0, 0, 0, 0,       /* AND NOT OR COND RIGHT FIELD */
	0,                      /* TEXPR (44) */
	1, 1, 1,		/* ICS RTC CRET */
	1, 1,			/* ADRFF ADRFL */
	0, 0, 0,		/* GETREG PUTREG NOTE (52) */
	0, 0, 0, 0,		/* KCOND KLABEL KTEST KJUMP (56) */
	0, 0, 0,		/* VAARG VASTART VAEND */
	0, 0, 1,		/* VREG FASGN ADRVS (62) */
	0, 1			/* DJUMP SELF */
};
Symbol rmap[16];
Node head;
...
Symbol mkfakereg(int set, int a2l) {
     static int n = 0;
     Symbol p;

     assert(eval.currentclosure);
     NEW0(p, FUNC);
     p->x.name = stringf("tr%d", n++);
     NEW0(p->x.regnode, PERM);
     p->x.regnode->number = -1;
     p->x.regnode->mask = 0;
     p->x.regnode->set = set;
     if (a2l)
	  eval.currentclosure->u.closure.fakeregs =
	       append(p, eval.currentclosure->u.closure.fakeregs);
     return p;
}
...
static int getrule(p, nt) Node p; int nt; {
     ...
     if (!rulenum) {
	  fprint(2, "no instruction-selection rule for ");
	  dumptree(p);
	  fprint(2, "\n");
	  assert(rulenum);
     }
...
int range(p, lo, hi) Node p; int lo, hi; {
     ...
     switch (p->op) {
     case ADRFFP: case ADDRFP: 
	  ck(s->x.offset >= lo && s->x.offset <= hi);
     case ADRFLP: case ADDRLP: 
	  ck(s->x.offset >= lo && s->x.offset <= hi);
     ...
     if (IR->x._isinstruction[rulenum] && p->x.emitted)
	  ...
	  for ((*IR->x._kids)(p, rulenum, kids); *fmt; fmt++)
		    ...
		    *bp++ = *fmt;
		    ...
		    assert(p->type && isstruct(p->type));
		    outs(tfAggregate(p->type));
	       }
	       else if (*fmt == 'Z') {
		    assert(p->type && isstruct(p->type));
		    print("%d", p->type->size);
		    ...
		    int i = *++fmt - '0';
		    if (i >= 0 && i < NELEMS(p->syms)) {
			 assert(p->syms[0]);
			 *bp++ = tfSparse(p->syms[0]->type);
		    }
	       } else if (*fmt == 'T') {
		    Type type = p->type;
		    Symbol typed = p->syms[RX];
		    int i;
		    for (i = 0; !type && !typed && i < NELEMS(kids) && kids[i]; i++)
			 if (kids[i]->type)
			      type = kids[i]->type;
			 else
			 if (kids[i]->syms[RX])
			      typed = kids[i]->syms[RX];
			 else if (kids[i]->syms[0])
			      typed = kids[i]->syms[0];
		    if (!type && typed)
			 type = typed->type;
		    assert(type);
		    outs(tfVcodeSuffix(type));
	       } else if (*fmt == 'S') {
		    Type type = NULL;
		    Symbol typed = NULL;
		    ...
		    if (kids[0]) {
			 type = kids[0]->type;
			 if (!type && kids[0]->syms[RX])
			      typed = kids[0]->syms[RX];
			 else if (!type && kids[0]->syms[0])
			      typed = kids[0]->syms[0];
		    }
		    if (!type && typed)
			 type = typed->type;
		    ...
		    if (type == unsignedchar)
			 ...
			 outs("us");
		    else if (type == signedchar || type == chartype)
			 outs("c");
		    ...
		    int i = *++fmt - '0';
		    if (i >= 0 && i < NELEMS(kids) 
			&& (*IR->x.isimmediate)(kids[i]))
			 *bp++ = 'i';
...
void emit(p) Node p; {
     for (; p; p = p->x.next) {
	  assert(p->x.registered || eval.currentclosure);
...
static void prelabel(p) Node p; {
     ...
     prelabel(p->kids[1]);
				...
				/* Fix references to specs */
     if (optype(p->op) == CS)	/* Rep of cspecs is P */ 
	  p->op = (generic(p->op) == CALL) ? CALLI : generic(p->op)+P;
     else if (optype(p->op) == VS) 
				/* Rep of vspecs is I */
	  p->op = (generic(p->op) == CALL) ? CALLI : generic(p->op)+I;

     if (NeedsReg[opindex(p->op)])
	  setreg(p, rmap[optype(p->op)]);
     switch (generic(p->op)) {
     case ADRFF: case ADRFL:
	  assert(0);		/* No free variables in static code */
...
/* tprelabel: restructure code for dynamic code templates */
extern Symbol tr;
static void tprelabel (Node p) {
     Node k0;
     ...
     switch (generic(p->op)) {
     case ADDRF: case ADDRL:
	  if (eval.fc && (p->syms[0]->type->op == VSPEC || p->syms[0]->lclp)) {
	       p->op = ADRVS;	/* Explicit vspecs are tricky  */
	       if (p->syms[0]->sclass == REGISTER)
		    p->syms[0]->sclass = AUTO;
	       setreg(p, tr);
	  } else if (p->syms[0]->sclass == REGISTER)
	       p->op = VREG+P;
	  break;
     case INDIR:
	  k0 = p->kids[0];
	  if (k0->op == VREG+P && p->op != INDIRB)
	       setreg(p, k0->syms[0]);
	  else if (!eval.fc 
		   && (generic(k0->op) == ADDRF || generic(k0->op) == ADDRL)) {
	       Symbol r; int t;
	       assert(p->syms[RX]);

	       t = (optype(p->op) == D || optype(p->op) == F) ? FREG : IREG;
	       r = mkfakereg(t, 0);
	       r->x.name = k0->syms[0]->name;

	       k0->op = VREG+P;
	       k0->syms[0] = k0->syms[RX] = r;
	       setreg(p, r);
	  }
	  break;
     case ASGN: {
	  int gk1 = generic(p->kids[1]->op);
	  k0 = p->kids[0];
	  if (k0->op == VREG+P) {
	       debug(fprint(2, "(cse=%x)\n", k0->syms[0]->u.t.cse));
	       rtarget(p, 1, k0->syms[0]);
	  } else if ((!eval.fc
		      && (generic(k0->op) == ADDRF || generic(k0->op) == ADDRL)
		      && (gk1 >= CVC && gk1 <= NEG
			  || gk1 >= ADD && gk1 <= MUL))
/*		     || (eval.fc && (generic(k0->op) == ADDRF
				     || generic(k0->op) == ADDRL)
			 && k0->syms[0]->lclp)
			 */
	       ) {
	       Symbol r; int t;
				/* Make fake reg having name of destination */
	       t = (optype(p->op) == D || optype(p->op) == F) ? FREG : IREG;
	       r = mkfakereg(t, 0);
	       r->x.name = k0->syms[0]->name;
				/* Change opcode; set dst of rhs to fake reg */
	       p->op = FASGN;
	       setreg(p->kids[1], r);
	  }
     }
...
static Node lastput[2][32];
static Node firstget[2][32];

#define nodeIsGetreg(p) (generic((p)->op) == GETREG)
#define nodeIsPutreg(p) (generic((p)->op) == PUTREG)
#define nodeIsCspec(p)  (generic((p)->op) == ICS)

#if 1
void nodeRemove (Node p) {
     Node n = p->x.prev; Node q = p->x.next;
     if (n) n->x.next = q;
     if (q) q->x.prev = n;
}
#else
...
static void checkfirstget (Node p) {
     Symbol s;
     int num, set;
     assert(p && (nodeIsCspec(p) || nodeIsGetreg(p)));
     if (nodeIsCspec(p)) {
	  if (p->op == ICSV) return;
	  assert(p->syms[RX] && p->syms[RX]->x.regnode);
	  s = p->syms[RX];
     } else {
	  assert(p->syms[0] && p->syms[0]->x.regnode);
	  s = p->syms[0];
     }
     num = s->x.regnode->number;
     set = s->x.regnode->set;

     if (firstget[set][num] == NULL)
	  firstget[set][num] = p;
     else if (nodeIsGetreg(p)) {
	  nodeRemove(p);
	  if (lastput[set][num]) {
	       nodeRemove(lastput[set][num]);
	       lastput[set][num] = NULL;
	  } else /* Should not have 2 getreg with no putreg in between */
	       assert(nodeIsCspec(firstget[set][num]));
     }
}
static void checklastput (Node p) {
     int num, set;
     assert(p->syms[0] && p->syms[0]->x.regnode);
     num = p->syms[0]->x.regnode->number;
     set = p->syms[0]->x.regnode->set;
     assert(!lastput[set][num]);
     lastput[set][num] = p;
}
static void docspec (int set, int num) {
     if (firstget[set][num] && lastput[set][num])
	  firstget[set][num] = lastput[set][num] = NULL;
     assert(!lastput[set][num]);
}
Node gen(forest) Node forest; {
     ...
     for (p = forest; p; p = p->x.next)
	  if (p->x.copy && p->x.kids[0]->syms[RX]->u.t.cse) {
	       ...
	       for (q = temp->u.t.cse; q; q = q->x.next)
			...
			|| (q->op == LABELV || q->op == JUMPV 
			    || generic(q->op)==RET || generic(q->op)==EQ 
			    || generic(q->op)==NE || generic(q->op)==LE 
			    || generic(q->op)==LT || generic(q->op)==GE 
			    || generic(q->op)==GT
			    || (generic(q->op) == CALL 
				&& dst->sclass != REGISTER)))
			 break;
     ...
     for (p = forest; p; p = p->x.next) {
	  ralloc(p);
	  if (p->x.listed && NeedsReg[opindex(p->op)] && rmap[optype(p->op)]) {
	       assert(generic(p->op) == CALL || generic(p->op) == LOAD
		    || generic(p->op) == ICS || generic(p->op) == CRET);
	       putreg(p->syms[RX],p);
	  }
     }
     if (eval.currentclosure && eval.fc) { /* strip redundant getreg/putreg */
	  for (j = 0; j < 2; j++)
	       for (i = 0; i < 32; i++)
		    lastput[j][i] = firstget[j][i] = 0;
	  for (p = forest; p; p = p->x.next) {
	       if (!p) continue;
	       if (nodeIsGetreg(p))
		    checkfirstget(p);
	       else if (nodeIsPutreg(p))
		    checklastput(p);
	       else if (nodeIsCspec(p)) {
		    for (j = 0; j < 2; j++)
			 for (i = 0; i < 32; i++)
			      docspec(j, i);
		    checkfirstget(p);
...
static inline void createnode (p, n, q) Node p, n, q;
{
  p->count = 1; 
  (*IR->x._label)(p);
  p->x.registered = 1; p->x.inst = 1; p->x.listed = 1;
  p->x.next = q;    p->x.prev = n;
  q->x.prev = p;    n->x.next = p;
}
static void putreg(r, n) Symbol r; Node n; {
     assert(r && r->x.regnode);
     freemask[r->x.regnode->set] |= r->x.regnode->mask;
     debug(dumpregs("(freeing %s)\n", r->x.name, NULL));

     if (eval.currentclosure && eval.fc && n && generic(n->op) != ICS) {
	  Node p;
	  assert(r->x.lastuse);
	  p = newnode(PUTREG, r->x.lastuse, NULL, r);
	  createnode(p,n,n->x.next);
     }
}
...
Symbol rs; unsigned rmask[]; {
     ...
     if (rs->x.wildcard == NULL) {
	  r = askfixedreg(rs);
	  if (r == NULL && eval.currentclosure)
	       r = mkfakereg(rs->x.regnode->set, 1);
	  return r;
     }
     for (i = 31; i >= 0; i--) {
	  r = rs->x.wildcard[i];
     ...
     if (eval.currentclosure)
	  return mkfakereg(r->x.regnode->set, 1);
     return NULL;
}
...
Symbol s; unsigned mask[]; Node p;  {
     ...
     if (eval.currentclosure && eval.fc) {
	  Node n = NULL;
	  if (generic(p->op) != ICS 
	      && !(generic(p->op) == INDIR
		   && generic(p->kids[0]->op) == ICS)) {
	       n = newnode(GETREG, p, NULL, r);
	       createnode(n,p->x.prev,p);
	  }
     }
     return r;
}
...
static void ralloc(p) Node p; {
     int i;
     unsigned mask[2];
     unsigned skip[2] = {0,0};	/* Keep track of source operands PUTREG'd,
				   so that if the same register is GETREG'd
				   we do not emit a redundant PUT/GET pair. */
     mask[0] = tmask[0];
     mask[1] = tmask[1];
     assert(p);
     ...
     for (i = 0; i < NELEMS(p->x.kids) && p->x.kids[i]; i++) {
	  ...
	  if (r->sclass != REGISTER && r->x.lastuse == kid) {
	       if (eval.fc)
		    skip[r->x.regnode->set] |= 1<<r->x.regnode->number;
	       putreg(r,p->x.prev);
	  }
     }
     if (!p->x.registered && NeedsReg[opindex(p->op)] && rmap[optype(p->op)]) {
	  ...
	  if (set->sclass != REGISTER) {
	       ...
	       r = getreg(set, mask, p);
	       if (eval.fc)
		    if (skip[r->x.regnode->set]&(1<<r->x.regnode->number)) {
				/* Remove PUT/GET pair */
			 int n = r->x.regnode->number;
			 Node k = p->x.prev, k2;
				/* k could be a cspec (ICS or INDIR(ICS)) */
			 if (k && k->op == GETREG) {
			      assert(k->syms[0]->x.regnode->number == n);
			      k2 = k->x.prev; nodeRemove(k); k = k2;
			      while (k && k->op == PUTREG) {
				   if (k->syms[0]->x.regnode->number == n) {
					nodeRemove(k);
					break;
				   }
				   k = k->x.prev;
...
Symbol r, tmp; Node last; {
     ...
     for (p = last->x.next; p != q; p = p->x.next) {
	  ralloc(p);
	  assert(!p->x.listed || !NeedsReg[opindex(p->op)] 
		 || !rmap[optype(p->op)]);
     }
}
...
unsigned regloc(p) Symbol p; {
     assert(p && p->sclass == REGISTER && p->sclass == REGISTER
	    && p->x.regnode);
