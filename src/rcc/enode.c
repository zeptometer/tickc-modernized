...
#define unquoted (eval.ticklevel && eval.unqtlevel && !eval.rtcp)
...
Tree call(f, fty, src) Tree f; Type fty; Coordinate src; {
     int varparam;		/* True if the current function takes a
     ...
     if (eval.ticklevel && isspec(rty))
	  ++eval.unqtlevel;	/* Each call to a function returing a spec type
				   opens one "unquoting level" */
     if (t != ')')
	  for (;;) {
	       Tree q;
	       q = pointer(expr1(0));
	       if (eval.ticklevel && eval.unqtlevel 
		   && q->scope > eval.ticklevel) {
				/* If  q is local to the `-expr, need to
				   allocate space for it in the closure and
				   generate a reference to this object */
		    q = closurefield(closureaddtree(q, CELCL));
	       }
	       varparam = 0;
	       if (proto && *proto && *proto != voidtype) {
		    Type aty;
		    q = value(q);
		    if (eval.ticklevel && q->type == voidtype) {
				/* CHECK: must add checks that it is a cspec */
			 assert(q->kids[0] && q->kids[0]->u.sym);
			 q->u.sym = q->kids[0]->u.sym;
			 varparam = 1;
		    } else {
			 aty = assign(*proto, q);
			 if (aty)
			      ...
			      error("type error in argument %d to %s; "
				    "found `%t' expected `%t'\n", n + 1, 
				    funcname(f), q->type, *proto);
...
/*			      assert(q->kids[0] && q->kids[0]->u.sym);
	       ...
	       if (!(eval.ticklevel ? dIR : IR)->wants_argb &&
		   isstruct(q->type))
		    if (iscallb(q))
			 q = addrof(q);
		    else {
     ...
     if (eval.ticklevel && isspec(rty)) {
	  --eval.unqtlevel;	/* Close this unquoting level */
	  assert(eval.unqtlevel >= 0);
     }
...
Tree calltree(f, ty, args, t3) Tree f, args; Type ty; Symbol t3; {
     Tree p;
     ...
     if (isstruct(ty))
	  ...
	  if ((isptr(ty) || p->type->size > ty->size)
	      && !(isspec(ty) && unqual(ty->type) == unqual(p->type)))
	       p = cast(p, ty);
     }
     if (eval.ticklevel) {
	  if (isspec(ty))
				/* An unquoted function does not synthesize
				   its scope from its arguments */
	       p->scope = 0;
	  if (eval.fc && !(eval.unqtlevel || eval.rtcp || isspec(ty)))
				/* This spec will call a function at runtime */
	       eval.leafp = 0;
     }
     return p;
}
...
Tree calllocaltree (Type ty, int spec) {
     Tree f, args;

     if (isarray(ty) || isstruct(ty)) {
	  f = idtree(findtcsym(eval.fc ? tcname.Localbf : tcname.Localb,
			       globals));
	  args = tree(ARGI, unsignedtype, 
		      consttree(ty->size, unsignedtype), NULL);
     } else {
	  f = idtree(findtcsym(eval.fc ? tcname.Localf : tcname.Local, 
			       globals));
	  args = tree(ARGI, unsignedtype, consttree(ttov(ty), unsignedtype), 
		      tree(ARGI, unsignedtype, consttree(spec, unsignedtype), 
			   NULL));
     }
     
     return calltree(f, mkvspec(ty), args, NULL);
}
Tree ticktree (s, ty) Symbol s; Type ty; {
     Tree p;

     p = tree(TEXPR+ttob(ty), mkcspec(ty), NULL, NULL);
     p->u.sym = s;
     return p;
}

/* rtcclean: traverses t looking for references to derived run-time constants 
   (ie run-time constant loop induction variables).
   Returns 0 if one is found (tree is not rtc-clean), 1 otherwise.
   All references to locals within t are appended to *l.
   References to free variables cause a compile-time error message. */
static int rtcclean (Tree * tp, List *l) {
     int retval = 1;
     Tree t;
     assert(tp);
     t = *tp;
     if (generic(t->op) == INDIR
	 && (generic(t->kids[0]->op) == ADDRL 
	     || generic(t->kids[0]->op) == ADDRF)) {
	  Symbol s = t->kids[0]->u.sym;
	  assert(s);

	  if (s->scope > eval.ticklevel && !s->rtcp) {
	       error("'%s' is dynamic: cannot be operand of '$'\n", s->name);
	       return 1;
	  }

	  if (s->rtcp) {
	       assert(s->drtcp); /* s is a derived run-time constant */
	       return 0;
	  } else 
	       *l = append(tp, *l);
	  return 1;
     }
     if (t->kids[0]) retval = rtcclean(&t->kids[0], l);
     if (t->kids[1]) retval &= rtcclean(&t->kids[1], l);
     return retval;
}
/* rtctreeinternal: changes tree *tp to be a run-time constant, adding the
   corresponding symbol to the closure state.  Returns the symbol corresponding
   to the run-time constant. */
static Symbol rtctreeinternal (Tree *tp) {
     Tree d, t;
     char *name = mkname(NAMERTC);
     Symbol s = install(name, &(eval.rtc), 0, FUNC);

     assert(tp);
     t = *tp;

     if (isspec(t->type)) {
	  error("illegal use of spec inside '$' expression\n");
	  d = tree(RTC+I, inttype, consttree(0, inttype), NULL);
     } else
	  d = tree(RTC+ttob(t->type), t->type, t, NULL);
...
void rtctree (Tree *t) {
     ...
     if (!rtcclean(t, &l)) {
	  ...
	  if (l) {
	       List lp = l;
	       do
		    rtctreeinternal((Tree *)lp->x);
	       while ((lp = lp->link) != l);
	  }
	  s = rtctreeinternal(t);
	  s->nrtcp = 1;
	  s->name = s->x.name = unparsetree((*t)->kids[0]);
	  return;
     } else
	  rtctreeinternal(t);
}
static Tree addtree(op, l, r) int op; Tree l, r; {
     ...
     if (eval.ticklevel
	 && ((isptr(l->type) && isspec(l->type->type) 
	      && (r->scope > eval.ticklevel || isvspec(r->type)))
	     || (isptr(r->type) && isspec(r->type->type) 
		 && (l->scope > eval.ticklevel || isvspec(l->type))))) {
	  error("binding time mismatch: pointer to [cv]spec/dynamic local\n");
	 ...
	 && (  (isconst(xty->type)    || !isconst(yty->type))
	  ...
	  if (isenum(lty) && rty == inttype
	      ||  isenum(rty) && lty == inttype) {
	       if (Aflag >= 1)
		    warning("assignment between `%t' and `%t' is "
			    "compiler-dependent\n", xty, yty);
	       return xty;
	  }
     }
				/* cspecs */
     if ((iscspec(xty) && iscspec(yty)
	  && eqtype(unqual(xty->type), unqual(yty->type), 1))
	 &&  ((isconst(xty->type) || !isconst(yty->type))
	      && (isvolatile(xty->type) || !isvolatile(yty->type))))
	  return xty;
				/* vspecs */
     if ((isvspec(xty) && isvspec(yty)
	  && eqtype(unqual(xty->type), unqual(yty->type), 1))
	 &&  ((isconst(xty->type) || !isconst(yty->type))
	      && (isvolatile(xty->type) || !isvolatile(yty->type))))
	  return xty;
...
Tree condtree(e, l, r) Tree e, l, r; {
     ...
     if (generic(e->op) == RTC) {
	  p = tree(KCOND, ty, ktest(e), 
		   tree(RIGHT, ty, cast(l, ty), cast(r, ty)));
     } else {
	  p = tree(COND, ty, cond(e), tree(RIGHT, ty, root(l), root(r)));
     }
...
Tree bittree(op, l, r) int op; Tree l, r; {
     ...
     if (op == MOD)
	  return simplify(op, ty, l, r);
     return cast(simplify(op, uty, l, r), ty);
}
...
static Tree subtree(op, l, r) int op; Tree l, r; {
     ...
     if (isarith(l->type) && isarith(r->type)) {
	  ...
	  l = simplify(SUB+U, unsignedtype, cast(l, unsignedtype),
		       cast(r, unsignedtype));
	  return simplify(DIV+I, inttype, cast(l, inttype), 
			  consttree(n, inttype));
     } else
	  typeerror(op, l, r);
     return simplify(op, ty, l, r);
...
Tree notetree(s) char *s; {
     Tree t = tree(NOTE, voidtype, NULL, NULL);
     t->u.sym = annotation(s);
     return t;
}
