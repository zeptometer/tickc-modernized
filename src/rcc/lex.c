...
static Symbol icon ARGS((unsigned long, int, int));
static void ppnumber ARGS((char *));

static int gettokinternal ARGS((void));
...
#define OUTPUT(x) cbufput( ( x ) )

int gettok() {
     int t = gettokinternal();
     ...
     if (cbe.emit && cbe.flush)
	  ...
	  case SHORT: 		OUTPUT("short "); 	break;
	  case INT: 		OUTPUT("int "); 	break;
	  case UNSIGNED: 	OUTPUT("unsigned "); 	break;
	  case VOID:		OUTPUT("void "); 	break;
	  case STRUCT: 		OUTPUT("struct "); 	break;
	  case UNION: 		OUTPUT("union "); 	break;
	  case ENUM: 		OUTPUT("enum "); 	break;
	  case LONG:		OUTPUT("long "); 	break;
	  case CSPEC:		OUTPUT("cspec "); 	break;
	  case VSPEC:		OUTPUT("vspec "); 	break;
	  case VALIST:		OUTPUT("va_list "); 	break;
	  case CONST:		OUTPUT("const "); 	break;
	  case VOLATILE:	OUTPUT("volatile "); 	break;
	  case TTICK:		OUTPUT("`"); 		break;
	  case TAT:		OUTPUT("@"); 		break;
	  case TDLR:		OUTPUT("$"); 		break;
	  case TSELFF:		OUTPUT("selff "); 	break;
	  case TSELFD:		OUTPUT("selfd "); 	break;
	  case TSELFI:		OUTPUT("selfi "); 	break;
	  case TSELFV:		OUTPUT("selfv "); 	break;
	  case TCOMPILE:	OUTPUT("compile "); 	break;
	  case TDECOMPILE:	OUTPUT("decompile "); 	break;
	  case TLOCAL:		OUTPUT("local "); 	break;
	  case TPARAM:		OUTPUT("param "); 	break;
	  case TPUSHI:		OUTPUT("pushi "); 	break;
	  case TPUSH:		OUTPUT("push "); 	break;
	  case TARG:		OUTPUT("arg "); 	break;
	  case TJUMP:		OUTPUT("jump "); 	break;
	  case TLABEL:		OUTPUT("label "); 	break;
	  case TFC:		OUTPUT("tc_fc ");	break;
	  case TSC:		OUTPUT("tc_sc "); 	break;
	  case ID:		OUTPUT(token); 		break;
	  case FCON:		OUTPUT(token); 		break;
	  case ICON:	{
	       assert(tsym);
	       OUTPUT(vtoa(tsym->type, tsym->u.c.v));
	  }
	       	break;
	  case SCON:
	       assert(tsym);
	       OUTPUT("\""); cbufputverbatim(tsym->u.c.v.p); OUTPUT("\""); 
	       	break;
	  case INCR:		OUTPUT("++"); 		break;
	  case DECR:		OUTPUT("--"); 		break;
	  case DEREF:		OUTPUT("->"); 		break;
	  case ANDAND:		OUTPUT("&&"); 		break;
	  case OROR:		OUTPUT("||"); 		break;
	  case LEQ:		OUTPUT("<="); 		break;
	  case EQL:		OUTPUT("=="); 		break;
	  case NEQ:		OUTPUT("!="); 		break;
	  case GEQ:		OUTPUT(">="); 		break;
	  case RSHIFT:		OUTPUT(">>"); 		break;
	  case LSHIFT:		OUTPUT("<<"); 		break;
	  case ELLIPSIS:	OUTPUT("..."); 		break;
	  case SIZEOF:		OUTPUT("sizeof "); 	break;
	  case AUTO:		OUTPUT("auto "); 	break;
	  case BREAK:		OUTPUT("break "); 	break;
	  case CASE:		OUTPUT("case "); 	break;
	  case CONTINUE:	OUTPUT("continue "); 	break;
	  case DEFAULT:		OUTPUT("default "); 	break;
	  case DO:		OUTPUT("do "); 		break;
	  case ELSE:		OUTPUT("else "); 	break;
	  case EXTERN:		OUTPUT("extern "); 	break;
	  case FOR:		OUTPUT("for "); 	break;
	  case GOTO:		OUTPUT("goto "); 	break;
	  case IF:		OUTPUT("if "); 		break;
	  case REGISTER:	OUTPUT("register "); 	break;
	  case RETURN:		OUTPUT("return "); 	break;
	  case SIGNED:		OUTPUT("signed "); 	break;
	  case STATIC:		OUTPUT("static "); 	break;
	  case SWITCH:		OUTPUT("switch "); 	break;
	  case TYPEDEF:		OUTPUT("typedef "); 	break;
	  case WHILE:		OUTPUT("while "); 	break;
	  case TVASTART:	OUTPUT("va_start ");	break;
	  case TVAARG:		OUTPUT("va_arg ");	break;
	  case TVAEND:		OUTPUT("va_end ");	break;
	  case '!': OUTPUT("!"); 	break;
	  case '%': OUTPUT("%"); 	break;
	  case '&': OUTPUT("&"); 	break;
	  case '(': OUTPUT("("); 	break;
...
static int gettokinternal() {
     for (;;) {
	  ...
	  case 'i':
	       ...
	       goto id;
	  case 'h': case 'k': case 'm': case 'n': case 'o':
	  ...
	  case '5': case '6': case '7': case '8': case '9': {
	       ...
	       if (*token == '0' && (*rcp == 'x' || *rcp == 'X')) {
		    ...
		    for (n = *token - '0'; map[*rcp]&DIGIT; ) {
			 int d = *rcp++ - '0';
			 if (n > (ULONG_MAX - (unsigned long)d)/10)
			      overflow = 1;
			 else
			      n = 10*n + d;
		    }
	  ...
	  case '\'': case '"': {
	       ...
	       *s++ = 0;
	       ...
	       if (cbuf[0] == '"') {
		    ...
		    if (s - cbuf > 3)
			 warning("excess characters in multibyte character "
				 "literal `%S' ignored\n", token, 
				 (char*)cp-token);
	  ...
	  case 's':
		   ...
		   && !(map[rcp[4]]&(DIGIT|LETTER))) {
		    switch (rcp[3]) {
		    case 'd': cp = rcp+4; return TSELFD;
		    case 'f': cp = rcp+4; return TSELFF;
		    case 'i': cp = rcp+4; return TSELFI;
		    case 'v': cp = rcp+4; return TSELFV;
		    default: goto id;
		    }
	       }
	       if (rcp[0] == 'h'
...
unsigned long n; int overflow, base; {
     if ((*cp=='u'||*cp=='U') && (cp[1]=='l'||cp[1]=='L')
	 ||  (*cp=='l'||*cp=='L') && (cp[1]=='u'||cp[1]=='U')) {
	  tval.type = unsignedlong;
	  cp += 2;
     } else if (*cp == 'u' || *cp == 'U') {
	  tval.type = n > UINT_MAX ? unsignedlong : unsignedtype;
	  cp += 1;
     } else if (*cp == 'l' || *cp == 'L') {
	  if (n > LONG_MAX)
	       tval.type = unsignedlong;
	       ...
	       tval.type = longtype;
	  cp += 1;
     } else if (base == 10 && n > LONG_MAX)
	  tval.type = unsignedlong;
	else if (n > INT_MAX)
	  tval.type = n <= UINT_MAX ? unsignedtype :
	       n <= LONG_MAX ? longtype : unsignedlong;
     else
	  tval.type = inttype;
     if (overflow) {
     ...
     tval.u.c.v.ul = 0;
     if (tval.type == unsignedlong)
	  tval.u.c.v.ul = n;
     else if (tval.type == longtype)
	  tval.u.c.v.l = (long)n;
     else if (isunsigned(tval.type))
	  tval.u.c.v.u = (unsigned)n;
     else
	  tval.u.c.v.i = (int)n;
     ppnumber("integer");
     return &tval;
...
/* fcon: modified to return string rep of floating point value in "token" */
static Symbol fcon() {
     char *end;
     ...
     errno = 0;
     tval.u.c.v.d = strtod(token, &end);

     strncpy(fbuf, token, end-token); *((char *)fbuf + (end - token)) = 0;
     token = fbuf;
